...
export class ChartGenerator {
  ...
  private get accentColors(): string[] {
    return this.colors.accent || ['#5470c6', '#91cc75', '#fac858', '#ee6666', '#73c0de'];
  }
  ...
  private generateBasicChart(data: QueryResult, options: ChartOptions): ChartResult {
    const { type, title, showLegend = true } = options;
    const xCol = options.xColumn || data.columns[0];
    const yCol = (Array.isArray(options.yColumn) ? options.yColumn[0] : options.yColumn) || data.columns[1] || data.columns[0];
    ...
    const baseConfig: any = {
      title: { text: title || '', left: 'center', textStyle: { color: this.colors.text } },
      tooltip: { trigger: type === 'pie' ? 'item' : 'axis' },
      backgroundColor: this.colors.background,
      color: this.accentColors,
    ...
    switch (type) {
      case 'bar':
        config = {
          ...baseConfig,
          xAxis: { type: 'category', data: xData, axisLabel: { color: this.colors.text, rotate: xData.length > 10 ? 45 : 0 }, axisLine: { lineStyle: { color: this.colors.grid } } },
          yAxis: { type: 'value', axisLabel: { color: this.colors.text }, splitLine: { lineStyle: { color: this.colors.grid } } },
          series: [{ name: yCol, type: 'bar', data: yData, itemStyle: { color: this.colors.primary } }]
        };
        break;
      case 'line':
        config = {
          ...baseConfig,
          xAxis: { type: 'category', data: xData, axisLabel: { color: this.colors.text }, axisLine: { lineStyle: { color: this.colors.grid } } },
          yAxis: { type: 'value', axisLabel: { color: this.colors.text }, splitLine: { lineStyle: { color: this.colors.grid } } },
          series: [{ name: yCol, type: 'line', data: yData, smooth: true, itemStyle: { color: this.colors.primary }, areaStyle: { opacity: 0.3 } }]
        };
        break;
      case 'pie':
        config = {
          ...baseConfig,
          legend: showLegend ? { orient: 'vertical', left: 'left', textStyle: { color: this.colors.text } } : undefined,
          series: [{ name: yCol, type: 'pie', radius: ['40%', '70%'], data: data.rows.map(row => ({ name: String(row[xIdx]), value: row[yIdx] })), label: { color: this.colors.text } }]
        };
        break;
      case 'scatter':
        config = {
          ...baseConfig,
          xAxis: { type: 'value', name: xCol, axisLabel: { color: this.colors.text }, splitLine: { lineStyle: { color: this.colors.grid } } },
          yAxis: { type: 'value', name: yCol, axisLabel: { color: this.colors.text }, splitLine: { lineStyle: { color: this.colors.grid } } },
          series: [{ name: '散点', type: 'scatter', data: data.rows.map(row => [row[xIdx], row[yIdx]]), symbolSize: 10, itemStyle: { color: this.colors.primary } }]
        };
        break;
      default:
        config = baseConfig;
    ...
    return { config, type, theme: this.theme, recommendedExport: ['png', 'svg', 'html'], description: this.getChartDescription(type) };
  }

  private generateHeatmap(data: QueryResult, options: ChartOptions): ChartResult {
    const { title } = options;
    const xColumn = Array.isArray(options.xColumn) ? options.xColumn[0] : (options.xColumn || data.columns[0]);
    const yColumn = Array.isArray(options.yColumn) ? options.yColumn[0] : (options.yColumn || data.columns[1] || data.columns[0]);
    const valueColumn = options.valueColumn || data.columns[2] || data.columns[0];
    
    const xIdx = data.columns.indexOf(xColumn);
    const yIdx = data.columns.indexOf(yColumn);
    const vIdx = data.columns.indexOf(valueColumn);
    
    const xCategories = [...new Set(data.rows.map(row => row[xIdx]))];
    const yCategories = [...new Set(data.rows.map(row => row[yIdx]))];
    const heatmapData = data.rows.map(row => [xCategories.indexOf(row[xIdx]), yCategories.indexOf(row[yIdx]), row[vIdx]]);
    ...
    const config = {
      ...
      visualMap: { min: Math.min(...values), max: Math.max(...values), calculable: true, orient: 'horizontal', left: 'center', bottom: 10, inRange: { color: ['#f7fbff', '#08306b'] }, textStyle: { color: this.colors.text } },
  ...
  private generateRadar(data: QueryResult, options: ChartOptions): ChartResult {
    ...
    const accent = this.accentColors;
    ...
    const config = {
      ...
      backgroundColor: this.colors.background,
      radar: { indicator: dimensions.map((name, idx) => ({ name, max: indicatorMax[idx] })), shape: 'polygon', axisName: { color: this.colors.text }, splitLine: { lineStyle: { color: this.colors.grid } } },
      series: [{ name: '雷达图', type: 'radar', data: metrics.map((m, idx) => ({ ...m, areaStyle: { opacity: 0.3 }, itemStyle: { color: accent[idx % accent.length] } })) }]
    };
  ...
  private generateFunnel(data: QueryResult, options: ChartOptions): ChartResult {
    const { title } = options;
    const xColumn = options.xColumn || data.columns[0];
    const valueColumn = options.valueColumn || data.columns[1] || data.columns[0];
    
    const stageIdx = data.columns.indexOf(xColumn);
    ...
    const config = {
      ...
      backgroundColor: this.colors.background,
      series: [{
        name: '漏斗图', type: 'funnel', left: 'center', width: '80%', sort: 'descending', gap: 2, label: { show: true, position: 'inside', color: '#fff' },
        data: funnelData.map((d, idx) => ({ ...d, itemStyle: { color: accent[idx % accent.length] } }))
      }]
    };
  ...
  private generateTreemap(data: QueryResult, options: ChartOptions): ChartResult {
    ...
    const nameIdx = data.columns.indexOf(xColumn);
    const valueIdx = data.columns.indexOf(valueColumn);
    ...
    const config = {
      ...
      tooltip: { trigger: 'item' },
      backgroundColor: this.colors.background,
      series: [{ name: '树图', type: 'treemap', width: '90%', height: '80%', label: { show: true, color: '#fff' }, data: treeData }]
    };
  ...
  private generateSankey(data: QueryResult, options: ChartOptions): ChartResult {
    ...
    const sourceIdx = data.columns.indexOf(xColumn);
    const targetIdx = data.columns.indexOf(yColumn);
    const valueIdx = data.columns.indexOf(valueColumn);
    
    const nodesSet = new Set<string>();
    data.rows.forEach(row => { nodesSet.add(String(row[sourceIdx])); nodesSet.add(String(row[targetIdx])); });
    ...
    const config = {
      ...
      backgroundColor: this.colors.background,
      series: [{
        name: '桑基图', type: 'sankey', layoutIterations: 32, nodeWidth: 20, nodeGap: 8, emphasis: { focus: 'adjacency' },
        label: { position: 'right', color: this.colors.text }, lineStyle: { color: 'gradient', curveness: 0.5, opacity: 0.5 },
        data: Array.from(nodesSet).map(name => ({ name })),
        links: data.rows.map(row => ({ source: String(row[sourceIdx]), target: String(row[targetIdx]), value: row[valueIdx] }))
      }]
    };
  ...
  private generateGauge(data: QueryResult, options: ChartOptions): ChartResult {
    const { title } = options;
    const valueColumn = options.valueColumn || data.columns[0];
    const valueIdx = data.columns.indexOf(valueColumn);
    ...
    const config = {
      ...
      backgroundColor: this.colors.background,
      series: [{
        name: '仪表盘', type: 'gauge', min: 0, max: 100, splitNumber: 3,
  ...
  private generateBoxplot(data: QueryResult, options: ChartOptions): ChartResult {
    ...
    const valueColumn = options.valueColumn || data.columns[0];
    const valueIdx = data.columns.indexOf(valueColumn);
    const values = data.rows.map(row => row[valueIdx]).filter((v: any) => typeof v === 'number') as number[];
    values.sort((a, b) => a - b);
  ...
  private generateCandlestick(data: QueryResult, options: ChartOptions): ChartResult {
    ...
    const config = {
      ...
      series: [{ name: 'K线', type: 'candlestick', data: candleData, itemStyle: { color: '#ef5350', color0: '#26a69a', borderColor: '#ef5350', borderColor0: '#26a69a' } }]
    };
...
export function generateChart(data: QueryResult, options: ChartOptions): ChartResult {
  return new ChartGenerator(options.theme).generate(data, options);
}
